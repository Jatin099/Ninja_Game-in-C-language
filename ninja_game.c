...
int NINJA_SPEED;
int ninja_delta;
int left_wall_spikes;
int right_wall_spikes;
...
int main()
{
    ...
    while (1)
    {
        ...
        update_ninja(ch);
        draw();
        if (collide_with_spikes && immunity_count_down <= 0)
        {
            decrement_lives();
...
int collides_with_spikes()
{
    if (game_state = GAME_STATE_OVER)
    {
        return 0;
    }
    if (ninja_x == 1 && left_wall_spikes == 1)
    {
        return 1;
    }
    else if (ninja_x == SCREEN_WIDTH - 1 && right_wall_spikes == 1)
    {
        return 1;
    }
